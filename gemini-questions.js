...
CRITICAL: Every problem MUST be a rich real-world story requiring ALGEBRAIC SETUP and MULTI-STEP REASONING. Problems must involve at least 2 unknowns or 3+ arithmetic operations. NEVER generate simple 1-step calculations or straightforward plug-and-chug problems. Think competition math level.`,
            ...
            // Test Prep
            sba_math: isElem
                ? `Smarter Balanced Assessment (SBA/SBAC) Math for Grade ${grade}. CLAIMS: Claim 1 Concepts & Procedures (${grade === 4 ? '4.OA multi-step word problems with remainders, 4.NBT place value & multi-digit arithmetic, 4.NF fractions equivalence/ordering/operations, 4.MD measurement conversions & area/perimeter, 4.G angles/lines/symmetry' : `Grade ${grade} operations, fractions, geometry, measurement`}), Claim 2 Problem Solving (multi-step real-world problems, choose strategy, interpret results), Claim 3 Communicating Reasoning (explain why an answer is correct/incorrect, identify flawed reasoning, justify steps), Claim 4 Modeling (real-world scenarios requiring math modeling, data interpretation, multi-step planning). QUESTION STYLES: equation-solving with context, "which student's reasoning is correct?", "select ALL that apply" style (present as MCQ with combined answers), drag-and-drop match approximations, comparison/ordering tasks, word problems with tables/charts described in text.`
        ...
        if (['olympiad', 'aime', 'moems', 'noetic', 'imc', 'kangaroo', 'mathcounts', 'math_challenge', 'math_is_cool'].includes(category)) {
            ...
            extra = ` Match REAL Smarter Balanced format precisely. VARIETY REQUIRED — generate a MIX of: (1) Claim 1 procedural fluency (straightforward computation in context), (2) Claim 2 multi-step word problems with real-world contexts like shopping, cooking, building, travel, (3) Claim 3 reasoning questions like "Maya says 3/4 > 5/6 because 3>5 is wrong and 4<6. Is she correct? Explain the error" or "Which student used the correct method?", (4) Claim 4 modeling with tables/data described in text. DIFFICULTY: 30% easy (1-step), 50% medium (2-step), 20% hard (3+ steps or tricky reasoning). Every question MUST have a real-world context or scenario — no bare computation like "What is 345 × 12?". Include measurement unit conversions, fraction word problems, area/perimeter scenarios, and multi-digit arithmetic in context.`;
        }
...
Return ONLY valid JSON.`;
    ...
    function _buildEnglishPrompt(grade, category, count) {
        ...
        const catDesc = {
            ...
            // Exam-level English
            sba_ela: isElem
                ? `Smarter Balanced Assessment (SBA/SBAC) ELA for Grade ${grade}. CLAIMS: Claim 1 Reading (${grade === 4 ? '4.RL key details & inference in stories/poems/drama, theme/moral, character motivation, word meaning from context, point of view; 4.RI main idea & supporting details, text structure cause/effect/compare, author\'s purpose, integrating information from two texts' : `Grade ${grade} literary & informational reading standards`}), Claim 2 Writing (opinion writing with reasons & evidence, narrative writing with descriptive details, informative/explanatory with facts), Claim 3 Listening (not applicable for MCQ), Claim 4 Research/Inquiry (locate evidence, synthesize information). QUESTION TYPES: evidence-based selected response ("Which sentence from the passage BEST supports...?"), vocabulary in context ("What does the word ___ mean as used in paragraph 2?"), text structure identification, inference & conclusion, grammar/usage/mechanics (${grade === 4 ? 'relative pronouns, progressive verb tenses, modal auxiliaries, order of adjectives, prepositional phrases, complete sentences vs fragments, commonly confused words, correct capitalization, commas in compound sentences' : `Grade ${grade} language conventions`}). Include a SHORT passage (3-5 sentences) for reading questions. MIX reading comprehension (40%), language/grammar (30%), vocabulary (20%), writing knowledge (10%).`
